...
export type Category =
  ...
  | "Aksesoris"
  | "Fullset";
export type ProductComponent = { productId: string; qty: number };
...
export const statusOf = (stock: number): Status =>
  stock <= 0 ? "Habis" : stock <= 3 ? "Terbatas" : "Tersedia";
...
function fromRow(row: any): Product {
  return {
    ...
    components: Array.isArray(row.components)
      ? row.components
          .filter((component: any) => typeof component?.productId === "string")
          .map((component: any) => ({
...
export async function removeProductRemote(id: string) {
  await deleteRows("products", `id=eq.${encodeURIComponent(id)}`);
}
...
export function useCatalog() {
  ...
  const addProduct = useCallback(async (x: Product) => {
    const created = await addProductRemote(x);
    products = [...products, created];
    setCatalog(products);
    return created;
  }, []);
  const updateProduct = useCallback(async (id: string, patch: Partial<Product>) => {
    const updated = await updateProductRemote(id, patch);
